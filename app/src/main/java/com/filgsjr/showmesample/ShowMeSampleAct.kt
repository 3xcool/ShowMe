...
class ShowMeSampleAct : AppCompatActivity(R.layout.activity_show_me_sample), AdapterView.OnItemSelectedListener, CoroutineScope {

  private val coroutineDispatcher = CoroutineContextProvider()
  private val job = Job()
  override val coroutineContext = job + coroutineDispatcher.main
  private val coroutineScope = CoroutineScope(coroutineContext)

  private var _binding: ActivityShowMeSampleBinding? = null
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    _binding = ActivityShowMeSampleBinding.inflate(layoutInflater)
...
//    val gsonConverter = GsonBodyConverter(Gson(), ShowMeAPILogModel()) //faster solution
    ...
    val httpSender1 :Sender? = ShowMeHttpSender.Builder(this)
      ...
      .active(false)  //todo set this to true in order to send the logs to the server
      .addHeaders(headers)
...
//    mShowMeDev.pruneAllWorks()
  ...
  private fun setSpinners() {
    ...
    binding.spinnerLogType.adapter = adapterLogCategory
    ...
    binding.spinnerWatcher.adapter = adapterWatcherCategory
    binding.spinnerWatcher.onItemSelectedListener = this
  }
  ...
  private fun setClickListeners() {
    ...
    binding.btnLog.setOnClickListener {
      setLogText(binding.etLogText.text.toString(), binding.spinnerLogType.selectedItemPosition, binding.spinnerWatcher.selectedItemPosition)
    }

    binding.btnReadLog.setOnClickListener {
      binding.tvLog.text = mShowMeDev.readLogFile() ?: "File is empty"
    }

    binding.btnGetUehFile.setOnClickListener {
      val uehContent = ueh.getUEHcontent()
      binding.tvLog.text = uehContent ?: "File is empty"
...
//        mShowMeDev.sendContent(it)  //if you just want to send to server
    ...
    binding.btnThrowUeh.setOnClickListener {
      throw Exception("Some test error")
    }

    binding.btnThrowUehCrash.setOnClickListener {
      binding.tvLog.text = someException  //this variable is null. Forcing NPE
    }
  ...
  private fun setExampleLog() {
    coroutineScope.launch {
      ...
      withContext(coroutineDispatcher.io){
        ...
        //Attention: the following logs will not appear in Summary because mShowMeProduction is set to show only Warning logs and above
